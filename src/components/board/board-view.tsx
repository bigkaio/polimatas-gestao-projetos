...
function ListColumn({
  ...
  return (
    <section
      ...
      className={clsx(
        "flex h-full w-72 shrink-0 flex-col rounded-2xl bg-slate-200/60 transition",
        blocked && "opacity-40"
      )}
    >
      <header className="flex items-center justify-between px-3 pb-1 pt-3">
        ...
        {showSum && sum > 0 ? (
          <span className="text-xs font-medium text-slate-500">{brl(sum)}</span>
        ) : null}
      </header>
      {blocked ? (
        <p className="mx-3 mb-1 rounded-md bg-red-100 px-2 py-1 text-xs text-red-700">🚫 {reason}</p>
      ) : null}
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-white px-4 py-2">
        ...
        {funnel ? (
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600">
            ...
            <span>
              Fechado no mês: <strong className="text-emerald-700">{brl(funnel.wonThisMonth)}</strong>
            </span>
