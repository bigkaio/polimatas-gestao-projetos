...
const eyebrow = "text-xs font-semibold uppercase tracking-widest text-accent";
...
export default async function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-bg text-fg-2">
      {/* Header fixo com blur (guia 5.1) */}
      <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3">
        <div className="mx-auto flex h-14 max-w-6xl items-center gap-4 rounded-2xl border border-line/10 bg-bg/80 px-4 backdrop-blur-md">
          ...
          <nav className="mx-auto hidden items-center gap-1 md:flex">
            {NAV.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="rounded-full px-3 py-1.5 text-sm font-medium text-fg-2 hover:bg-tint/10 hover:text-fg"
              >
                {item.label}
              </a>
            ))}
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            ...
            {session ? (
              <Link
                href={appHref}
                className="rounded-full bg-accent-solid px-4 py-1.5 text-sm font-semibold text-accent-fg hover:bg-accent-solid-hover"
              >
                Abrir o sistema
              </Link>
            ) : (
              <>
                <Link
                  href="/login"
                  className="rounded-full border border-line/15 px-4 py-1.5 text-sm font-medium text-fg-2 hover:bg-tint/10"
                >
                  Entrar
                </Link>
      ...
      <section className="hero-gradient px-4 pb-20 pt-32 text-center">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/logo.svg" alt="Polímatas" className="mx-auto h-24 w-auto" />
        <p className={`mt-6 ${eyebrow}`}>Gestão de vendas e projetos</p>
        <h1 className="mx-auto mt-3 max-w-3xl text-4xl font-light tracking-tight text-fg md:text-6xl">
          Da venda fechada ao projeto entregue,{" "}
          <span className="text-accent">sem repasse manual</span>.
        </h1>
        <p className="mx-auto mt-5 max-w-2xl text-lg text-fg-2">
          O Polímatas Flow conecta o funil comercial à execução em dois quadros integrados — com
          automações que você mesmo cria e regras de compliance que bloqueiam de verdade.
        </p>
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          <Link
            href={appHref}
            className="rounded-full bg-accent-solid px-6 py-3 text-sm font-semibold text-accent-fg hover:bg-accent-solid-hover"
          >
            {session ? "Abrir o sistema" : "Acessar o sistema"}
          </Link>
        </div>
        <p className="mt-6 text-xs text-fg-4">
      ...
      <section id="sobre" className="mx-auto max-w-5xl scroll-mt-24 px-4 py-20">
        ...
        <h2 className="mt-2 max-w-2xl text-3xl font-light tracking-tight text-fg md:text-4xl">
          Um único fluxo, do <span className="text-accent">Lead</span> ao{" "}
          <span className="text-accent">Concluído</span>
        </h2>
        <div className="mt-6 grid gap-4 text-fg-2 md:grid-cols-2">
          <p>
          ...
          <p>
            ...
            <strong className="text-fg">Pipeline de Vendas</strong> (Lead → Qualificação → Proposta
            → Negociação → Fechado/Perdido) e{" "}
            <strong className="text-fg">Pipeline de Projetos</strong> (Backlog → Em andamento →
      ...
      {/* Funcionalidades */}
      <section id="funcionalidades" className="border-t border-line/10 bg-bg">
        <div className="mx-auto max-w-5xl scroll-mt-24 px-4 py-20">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {[
              ...
              <div key={f.title} className="rounded-2xl border border-line/10 bg-surface p-5">
                <span className="flex h-10 w-10 items-center justify-center rounded-full border border-line/15 text-accent">
                  {f.icon}
                </span>
                <h3 className="mt-3 font-medium text-fg">
                  {f.title} <span className="text-accent">{f.cyan}</span>
                </h3>
                <p className="mt-2 text-sm text-fg-3">{f.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="como-funciona" className="mx-auto max-w-5xl scroll-mt-24 px-4 py-20">
        ...
        <ol className="mt-8 space-y-6 border-l border-line/10 pl-6">
          {[
            [
              "Registre a oportunidade",
              "O vendedor cria o card com cliente, valor e previsão — e move pelo funil conforme a negociação avança.",
            ],
            [
              "Feche a venda",
              "Ao arrastar para Fechado, o sistema confirma e mostra o que será criado. O motivo de perda é obrigatório no caminho contrário.",
            ],
            [
              "O projeto nasce sozinho",
              "Um card aparece no Backlog de Projetos herdando os dados da venda, sem retrabalho e sem duplicar — a mesma venda nunca gera dois projetos.",
            ],
            [
              "Execute com garantias",
              "Checklist com prazos obrigatórios, automações movendo e avisando, e compliance impedindo o que sair do padrão.",
            ],
          ].map(([title, text], i) => (
            <li key={title} className="relative">
              <span className="absolute -left-[35px] flex h-5 w-5 items-center justify-center rounded-full bg-accent-solid text-[10px] font-bold text-accent-fg">
                {i + 1}
              </span>
              <h3 className="font-medium text-fg">{title}</h3>
              <p className="mt-1 text-sm text-fg-3">{text}</p>
            </li>
          ))}
        </ol>
      </section>
      ...
      {/* Tecnologia */}
      <section id="tecnologia" className="border-t border-line/10 bg-bg">
        <div className="mx-auto max-w-5xl scroll-mt-24 px-4 py-20">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {["Next.js", "TypeScript", "Prisma", "PostgreSQL", "Tailwind", "Vercel"].map((t) => (
              <span
                key={t}
                className="rounded-full border border-line/10 px-3 py-1 text-sm text-fg-3"
              >
                {t}
              </span>
            ))}
      ...
      <section className="px-4 py-24 text-center">
        <h2 className="text-3xl font-light tracking-tight text-fg md:text-4xl">
          Pronto para ver o fluxo <span className="text-accent">rodando</span>?
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-fg-3">
          Entre com uma das credenciais de demonstração e acompanhe uma venda virar projeto.
        </p>
        <Link
          href={appHref}
          className="mt-7 inline-block rounded-full bg-accent-solid px-8 py-3 text-sm font-semibold text-accent-fg hover:bg-accent-solid-hover"
        >
          {session ? "Abrir o sistema" : "Acessar o sistema"}
        </Link>
      ...
      <footer className="border-t border-line/10 px-4 py-8">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3 text-sm text-fg-4">
